...
# 🔵⋯ v0.16.0 (2026-08-12) — #cm-45, #cm-44
...
- 2026-08-12 - feat (technical) | the tab context menu re-evaluates **Fork Conversation** while the agent index is still answering, instead of leaving the item stuck greyed out if you opened the menu early (#cm-45)
...
- 2026-08-12 - feat (technical) (#cm-45) | bonsplit subtree ported `10563e2f → 529913b7`. Upstream carries bonsplit as a `vendor/bonsplit` submodule, so their catch-up is one gitlink SHA and ours is a content merge — each file three-way merged against base, ours and theirs. `TabBarView` merged with **zero** conflicts despite ~314 fork lines against upstream's 509, because the two sides edited different regions; all six conflicts were in tests and none were semantic. Closes the stub the 2026-08 sync commit deferred: `tabContextForkConversationAvailabilityRefreshHandler` is now declared in `BonsplitController`, invoked from `TabBarView`, and set by cmux — a loop that was previously declared-but-never-invoked, and therefore inert (#cm-45)
