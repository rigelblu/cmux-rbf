...
- **ghostty pins one exact Zig version, and nothing here hardcodes which.** `rbf/scripts/lib/rbf-zig.sh` reads it from `ghostty/build.zig.zon` through upstream's `ghostty_minimum_zig_version`, the same function `scripts/build-ghostty-cli-helper.sh` derives `ZIG_REQUIRED` from, then exports both `CMUX_ZIG` and `PATH`. `make install-rbf` enforces it (`--required`) before building; `dev.sh` only warns, since a cached GhosttyKit may carry the build there. **Since the 2026-08 upstream sync the floor is 0.16.0** — it was 0.15.2, and the sync moved ghostty to a tree using the `@Int` builtin that 0.15.2 rejects. A stock `brew install zig` currently gives 0.16.0, so PATH usually satisfies it now; that is the reverse of the situation the pin was written for, which is exactly why the version is derived rather than typed.
