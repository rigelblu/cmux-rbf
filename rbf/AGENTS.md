...
- **ghostty declares a minimum compatible Zig series, and setup manages it.** `scripts/zig-toolchain.sh` derives the floor from `ghostty/build.zig.zon`, accepts a complete stable compiler in the same major/minor series at or above that patch, and is the one resolver used by setup, tagged reloads, the Ghostty CLI helper, and RBF build/install entrypoints. `make setup` reuses a compatible override, checkout-local compiler, or compiler anywhere on `PATH`; when none exists it downloads the verified minimum into ignored `.cmux-tools/`. Later build entrypoints fail early with the same diagnosis instead of selecting a different compiler. `CMUX_ZIG` is explicit and fail-closed: fix or unset an invalid override; it never silently falls back.
