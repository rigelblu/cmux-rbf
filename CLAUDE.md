...
A first pass ends when the change is implemented, the tagged build succeeded on the pushed HEAD, focused tests ran, and the PR is open. Then hand off to the user. Do not sit in the main conversation running speculative review passes after that point. <!-- cmux-rbf: pruned upstream text — removed the Vercel preview URL requirement and 'watching CI' — no Vercel deploys and no CI in this fork, see rbf/AGENTS.md. Reject this hunk on upstream sync. -->

Do not launch a background review agent (`$autoreview`, `codex review`, `claude review`, or a judge loop) by default. Second-model review is explicit user opt-in in the current conversation; an implementation request, open PR, closeout, or handoff is not that opt-in. <!-- cmux-rbf: pruned upstream text — removed 'CI failure' from the not-an-opt-in list and the sentence deferring to GitHub checks and automatic review bots — no checks or bots ever run here, see rbf/AGENTS.md. Reject this hunk on upstream sync. -->
...
- **Localize every user-facing string** (`cmux-localization`): `String(localized:)` with keys in `Resources/Localizable.xcstrings`, plus every web message catalog — **all 20 locales under `web/messages/`, not just `en.json`/`ja.json`**. <!-- cmux-rbf: pruned upstream text — upstream said en/ja; that is two of twenty here and would badly under-scope a localization audit. Reject this hunk on upstream sync. --> A localization audit is required for any UI, Settings, menu, schema, docs, or help-text change, and the handoff must state what was audited.
...
- **Test wiring** (`cmux-testing`): a `.swift` file in `cmuxTests/` without a `PBXFileReference` + `PBXSourcesBuildPhase` entry is silently skipped, and `xcodebuild test` passes with "Executed 0 tests". **Run `./scripts/lint-pbxproj-test-wiring.sh` yourself; nothing runs it for you.** <!-- cmux-rbf: pruned upstream text — removed 'bot reviews' and the `workflow-guard-tests` job — no CI and no review bots here, see rbf/AGENTS.md. Reject this hunk on upstream sync. -->
- **SPM package groups** (`cmux-architecture`): packages live under `Packages/{Shared,iOS,macOS}/<pkg>` and the workspace mirrors that folder shape. To move one, `git mv` the directory then `python3 scripts/check-workspace-package-groups.py --write`. Never hand-edit workspace group membership. **Run `python3 scripts/check-workspace-package-groups.py --check` yourself after any package move — nothing runs it for you.**
- **Do not gitignore cmux-owned `Package.resolved`.** SwiftPM resolution changes must show in PR diffs; package-local lockfiles are not replaced by the root one. **Run `python3 scripts/check-package-resolved-policy.py` yourself after any SwiftPM change — nothing runs it for you.** <!-- cmux-rbf: pruned upstream text — was 'fails on drift', implying CI — no CI here, see rbf/AGENTS.md. Reject this hunk on upstream sync. -->
...
- **Foundation, SwiftUI, AttributeGraph, and WebKit semantics change between macOS major versions.** `URL(fileURLWithPath: "/").deletingLastPathComponent().path` returns `"/.."` on macOS 14 and 15 but `"/"` on macOS 26 (https://github.com/manaflow-ai/cmux/issues/4529); Every maintainer's dev machine was on the fixed side while every reporter was on the broken side. Test on the reporter's macOS before declaring a repro disproven. <!-- cmux-rbf: pruned upstream text — removed the CI reference and the AWS M4 Pro builder fleet — not reachable from this fork, see rbf/AGENTS.md. Reject this hunk on upstream sync. -->
